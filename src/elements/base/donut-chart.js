...
export const DonutChartBase = class extends utilsMixin(dataSourceMixin(BaseElement)) {
		...
		this.sections
			...
			.attrTween('d', d => {
				...
				return t => {
					return d3.arc()
					.innerRadius(this.innerRadius)
					.outerRadius(this.outerRadius)(interpolate(t));
				}
			})
	}
	...
	createChartData() {
		...
		columnsToPlot.forEach(column=>{
			const columnIndex = this.cols.findIndex(item=>item.label === column);
