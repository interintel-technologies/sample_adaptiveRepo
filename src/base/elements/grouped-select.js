...
export const GroupedSelectBase = class extends utilsMixin(SerializableElement) {
  static get is() {
  ...
  _dataJoined(data) {
    ...
    return data.filter(function(item, index) {
      return !(index === 0 ||
                  ...
                  (this.cols.length && this.cols[index]['type'] === 'href'));
    }).join(' ');
  }
  ...
  firstUpdated(changedProperties) {
    super.firstUpdated(changedProperties);
      ...
      if (Object.keys(this.rows).length > 0 && ! self.dropdownValue) {
