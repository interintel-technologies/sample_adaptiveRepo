...
export const DataListElementBase = class extends utilsMixin(mqttMixin(SerializableElement)) {
  static get is() {
  ...
  updated(changedProperties) {
    if (changedProperties.has('e')) {
        console.log("this e in d base ",this.e);
  ...
  _handlePageChanged(event) {
    ...
    console.log("handling page change ",event.detail['page']);
  ...
  generateCSV() {
    ...
    const headers = this.columns.filter(col=>col['type']!='href').map(function(i) {
      return i['label'];
    });
    ...
    this.rows.forEach(function(row) {
      ...
      for (let i = 0; i < self.columns.length; i++) {
        const colType = self.columns[i]['type'];
        const columnKey = self.columns[i]['header'];
  ...
  /**
     ...
     */
  _validateType(x) {
    return (x && 'type' in x) ? x['type'].toLowerCase() : 'table';
  }

  _validateAlert(x) {
    return (x && 'alert' in x) ? x['alert'] : false;
  }

  _validateState(x) {
    return x &&  x.state ? x.state : {};
  }
  ...
  //   return (x && 'page_limits' in x) ? x['page_limits'] : [];
  // }
  ...
  generatePDF() {
    ...
    this.rows.forEach(function(row) {
      const rowArray = [];
        ...
        if (self.columns[i]['type']!='href') {
         
          rowArray.push(row[columnKey]);
        }
      }
      rows.push(rowArray);
    });
    // const filt=this.columns.filter(col=>col['type']!='href');
    // console.log(filt)
    // filt.map(function(i){
    //  console.log(i['header']) 
    // })
    const table = new AutoTable(doc);
    table.autoTable(this.columns.filter(col=>col['type']!='href').map(function(i) {
      return i['header'];
    }), rows, {
  ...
  validate() {
    ...
    if(this.selectable){
      ...
      console.log("actionRow ",this.actionRow);
